...
Auto Vacancy stores editable CV Drafts in Convex and renders immutable PDF Versions inside the application. The renderer follows the `CvInput` and `CvProfile` contract from `/Users/abeldebruijn/Documents/GitHub/job-applications/src/job_application_ts`, but the app implements the mapping and PDFKit rendering locally instead of shelling out to that project's CLI.
